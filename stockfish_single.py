import chess
import chess.engine
import chess.pgn
import json
import os

from utils.winPercentage import get_line_win_percentage
from utils.accuracy import get_move_accuracy, get_player_accuracy, calc_weights
from utils.estimateElo import get_position_cp, get_players_average_cpl, get_elo_from_rating_and_cpl
from utils.moveClassification import basic_move_classification, is_perfect_move, is_splendid_move
from utils.chess_utils import extract_clock_time, get_phase


engine = chess.engine.SimpleEngine.popen_uci("engine/stockfish_18")
depth = 22
pv = 2
JSON_FILE = "rastone98.json"
with open("openings_by_fen.json") as f:
    openings = json.load(f)
# ===== CARICA UNA SOLA PARTITA =====
with open("games/rastone98/game34.pgn") as f:
    game = chess.pgn.read_game(f)
w_acc = []
b_acc = []
positions_cp = []
board = game.board()
uci_moves = []
moves_data = []
opening_name_global = None
opening_locked = False
MAX_OPENING_MOVES = 10
node = game
index = 0
INITIAL_TIME = 600
prev_time_white = INITIAL_TIME
prev_time_black = INITIAL_TIME
current_phase = "opening"
# ===== CICLO MOSSE =====
while not node.is_end():
    next_node = node.variation(0)
    move = next_node.move
    comment = next_node.comment
    player = board.turn
    current_time = extract_clock_time(comment)
    time_spent = None
    if current_time is not None:
        if player == chess.WHITE:
            time_spent = prev_time_white - current_time
            prev_time_white = current_time
        else:
            time_spent = prev_time_black - current_time
            prev_time_black = current_time
    if time_spent is not None:
        time_spent = max(time_spent, 0)
    if board.is_game_over():
        break
    is_forced = len(list(board.legal_moves)) == 1
    info_before = engine.analyse(board, chess.engine.Limit(depth=depth), multipv=pv)
    win_before = get_line_win_percentage(info_before[0])
    cp_before = get_position_cp(info_before[0])
    positions_cp.append(cp_before)
    best_move = info_before[0]["pv"][0]
    prev_board = board.copy()
    board.push(move)
    phase_now = get_phase(board)
    if current_phase == "opening" and phase_now != "opening":
        current_phase = "middlegame"
    elif current_phase == "middlegame" and phase_now == "endgame":
        current_phase = "endgame"
    if not opening_locked and index < MAX_OPENING_MOVES:
        fen = board.fen().split(' ')[0]
        opening_name = openings.get(fen)
        if opening_name:
            opening_name_global = opening_name
        else:
            opening_locked = True
    info_after = engine.analyse(board, chess.engine.Limit(depth=depth), multipv=pv)
    score_after = info_after[0]["score"].white()
    if score_after.is_mate():
        win_after = 100.0 if score_after.mate() > 0 else 0.0
    else:
        win_after = get_line_win_percentage(info_after[0])
    accuracy = get_move_accuracy(win_before, win_after, player)
    alt_win = None
    if len(info_before) > 1:
        alt_win = get_line_win_percentage(info_before[1])
    move_class = None
    if is_forced:
        move_class = "Forced"
    if move_class is None:
        fen = board.fen().split(' ')[0]
        if openings.get(fen):
            move_class = "Opening"
    if move_class is None and alt_win is not None:
        if is_splendid_move(
            win_before,
            win_after,
            player,
            move.uci(),
            [m.uci() for m in info_before[0]["pv"][1:]],
            prev_board.fen(),
            alt_win
        ):
            move_class = "Splendid"
    if move_class is None and alt_win is not None:
        if is_perfect_move(
            win_before,
            win_after,
            player,
            alt_win,
            None,
            None
        ):
            move_class = "Perfect"
    if move_class is None and move == best_move:
        move_class = "Best"
    if move_class is None:
        move_class = basic_move_classification(win_before, win_after, player)
    moves_data.append({
        "san": prev_board.san(move),
        "classification": move_class,
        "time_spent": time_spent,
        "phase": current_phase
    })
    if player == chess.WHITE:
        w_acc.append(accuracy)
    else:
        b_acc.append(accuracy)
    uci_moves.append(move.uci())
    node = next_node
    index += 1

engine.close()
# ===== METRICHE FINALI =====
w_weights = calc_weights(w_acc)
b_weights = calc_weights(b_acc)
white_accuracy = get_player_accuracy(w_acc, w_weights)
black_accuracy = get_player_accuracy(b_acc, b_weights)
avg_cpl = get_players_average_cpl(positions_cp)
white_rating = int(game.headers.get("WhiteElo", 0) or 0)
black_rating = int(game.headers.get("BlackElo", 0) or 0)
white_elo = get_elo_from_rating_and_cpl(avg_cpl["whiteCpl"], white_rating or black_rating)
black_elo = get_elo_from_rating_and_cpl(avg_cpl["blackCpl"], black_rating or white_rating)
game_data = {
    "game_id": "AUTO",
    "white": game.headers.get("White", ""),
    "black": game.headers.get("Black", ""),
    "white_elo": white_rating,
    "black_elo": black_rating,
    "white_rating": round(white_elo),
    "black_rating": round(black_elo),
    "white_accuracy": round(white_accuracy, 1),
    "black_accuracy": round(black_accuracy, 1),
    "opening": opening_name_global or "",
    "engine": "stockfish18",
    "depth": depth,
    "pv": pv,
    "result": game.headers.get("Result", ""),
    "date": game.headers.get("Date", ""),
    "moves": moves_data
}
# ===== APPEND JSON =====
if os.path.exists(JSON_FILE):
    with open(JSON_FILE, "r") as f:
        all_games = json.load(f)
else:
    all_games = []
existing_ids = [int(g["game_id"]) for g in all_games] if all_games else []
next_id = str(max(existing_ids) + 1 if existing_ids else 1).zfill(5)
game_data["game_id"] = next_id
all_games.append(game_data)
with open(JSON_FILE, "w") as f:
    json.dump(all_games, f, indent=2)
print(f"Partita {next_id} aggiunta")